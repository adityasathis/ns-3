...
def calculate_qos_metrics(file_path):
    ...
    AverageThroughput = np.sum(np.array(achieved_throughputs)) / len(np.array(achieved_throughputs))
    AverageDelay = np.sum(np.array(achieved_delays)) / len(np.array(achieved_delays))
