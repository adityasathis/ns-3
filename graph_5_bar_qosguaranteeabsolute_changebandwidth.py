...
for bw in bandwidth:
    for capc in capc_configs:
        file_paths = [f'nru-csv/ip/changebandwidth-gnb6-ap0-ut10-ratio1111-numerology0-bandwidth{int(bw/1e6)}-capc{capc}-simtime1-run{i}.csv' for i in range(num_runs)]
        throughputs = []
        delays = []
...
x = np.arange(len(bandwidth))  # the label locations
...
ax[0].set_xticks(x)
ax[0].set_xticklabels(bandwidth)
ax[0].legend()
ax[0].grid(True)
...
ax[1].set_xticks(x)
ax[1].set_xticklabels(bandwidth)
ax[1].legend()
ax[1].grid(True)
